...
TCHAIN_ROOT = C:\ARM_Toolchain\CodeSourcery_gcc_4_6_3\Sourcery_CodeBench_Lite_for_ARM_EABI\bin\\
...
TARGET = MySportTracker

# Application directory defintions
CMSIS_DIR				= ./CMSIS
DEVICE_DIR				= ./device
FILESYS_DIR				= ./FS
GPS_DIR					= ./GPS
HWDRV_DIR				= ./HwDrv
RTOS_DIR				= ./RTOS
RTOS_PORT_DIR			= $(RTOS_DIR)/PORT/GCC
STM_PERIPH_LIB_SRC_DIR	= ./StdPeriphLib/src
STM_PERIPH_LIB_INC_DIR	= ./StdPeriphLib/inc
SYSTEM_DIR				= ./System
TRACKER_DIR				= ./Tracker
UTILITY_DIR				= ./Utility
...
# STM basic files
SRC += $(DEVICE_DIR)/stm32f10x_it.c
SRC += $(DEVICE_DIR)/system_stm32f10x.c
SRC += $(CMSIS_DIR)/core_cm3.c

# GPS source files
SRC += $(GPS_DIR)/gps.c
SRC += $(GPS_DIR)/gpsaltitude.c
SRC += $(GPS_DIR)/gpsdatetime.c
SRC += $(GPS_DIR)/gpslocation.c
SRC += $(GPS_DIR)/gpsstate.c
SRC += $(GPS_DIR)/nmeadll.c
SRC += $(GPS_DIR)/nmeaparser.c
SRC += $(GPS_DIR)/nmeaparsercfg.c

# RTOS source files
SRC += RTOS_Hooks.c
SRC += $(RTOS_DIR)/os_core.c
SRC += $(RTOS_DIR)/os_flag.c
SRC += $(RTOS_DIR)/os_mbox.c
SRC += $(RTOS_DIR)/os_mem.c
SRC += $(RTOS_DIR)/os_mutex.c
SRC += $(RTOS_DIR)/os_q.c
SRC += $(RTOS_DIR)/os_sem.c
SRC += $(RTOS_DIR)/os_task.c
SRC += $(RTOS_DIR)/os_time.c
SRC += $(RTOS_DIR)/os_tmr.c

SRC += $(RTOS_PORT_DIR)/os_cpu_c.c
SRC += $(RTOS_PORT_DIR)/os_dbg.c

# Std Peripheral Library
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/misc.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_adc.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_bkp.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_can.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_cec.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_crc.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_dac.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_dbgmcu.c
SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_dma.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_exti.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_flash.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_fsmc.c
SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_gpio.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_i2c.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_iwdg.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_pwr.c
SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_rcc.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_rtc.c
SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_sdio.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_spi.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_tim.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_usart.c
#SRC += $(STM_PERIPH_LIB_SRC_DIR)/stm32f10x_wwdg.c

# HW Drivers
SRC += $(HWDRV_DIR)/dio.c
SRC += $(HWDRV_DIR)/DIOConfig.c
SRC += $(HWDRV_DIR)/huart.c
SRC += $(HWDRV_DIR)/sdcard.c

SRC += $(FILESYS_DIR)/diskio.c
SRC += $(FILESYS_DIR)/ff.c

SRC += $(TRACKER_DIR)/gpxlogger.c
SRC += $(TRACKER_DIR)/tracker.c
SRC += $(SYSTEM_DIR)/systime.c
SRC += $(UTILITY_DIR)/utility.c
...
ASRC = $(DEVICE_DIR)/startup_stm32f10x_hd.s
ASRC += $(RTOS_PORT_DIR)/os_cpu_a.s
...
EXTRAINCDIRS += $(DEVICE_DIR)
EXTRAINCDIRS += $(FILESYS_DIR)
EXTRAINCDIRS += $(GPS_DIR)
EXTRAINCDIRS += $(HWDRV_DIR)
EXTRAINCDIRS += $(RTOS_DIR)
EXTRAINCDIRS += $(RTOS_PORT_DIR)
EXTRAINCDIRS += $(STM_PERIPH_LIB_SRC_DIR)
EXTRAINCDIRS += $(STM_PERIPH_LIB_INC_DIR)
EXTRAINCDIRS += $(SYSTEM_DIR)
EXTRAINCDIRS += $(TRACKER_DIR)
EXTRAINCDIRS += $(UTILITY_DIR)
...
#CFLAGS += -mfpu=fpv4-sp-d16 -mfloat-abi=hard -fno-strict-aliasing -Wdouble-promotion
######################################################
CFLAGS += $(CDEFS)
